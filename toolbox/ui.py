...
class UI(QDialog):
    ...
    def populate_models(self, models_dir: Path):
        ...
        self.repopulate_box(
            self.encoder_box, [(f.parent.name, f) for f in encoder_fpaths]
        )
        ...
        self.repopulate_box(
            self.synthesizer_box, [(f.parent.name, f) for f in synthesizer_fpaths]
        )

        # Vocoder
        vocoder_fpaths = list(models_dir.glob("*/vocoder.pt"))
        vocoder_items = [(f.parent.name, f) for f in vocoder_fpaths] + [
            ("Griffin-Lim", None)
        ]
    ...
    def __init__(self):
        ...
        super().__init__(None)
        self.setWindowTitle("Ajay toolbox")
